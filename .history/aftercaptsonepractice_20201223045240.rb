...
WHERE stopa.name = 'Craiglockhart' AND stopd.name = 'Lochend'
 ...
 select distinct actor.name from actor, movie, casting  where  movieid = movie.id and  actorid = actor.id  and movie.id = any (select id from movie where director = 'Art Garfunkel')